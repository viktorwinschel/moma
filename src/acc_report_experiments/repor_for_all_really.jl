...
Pattern (kategorisch):")
    ...
    println("  Investor           : Zahlung von ", b.amount, " zwischen Agenten mit Konto ", b.debit.id, " (Sender) und ", b.credit.id, " (Empfänger)")
...
function example_macro(agts, books)
    ...
    for i in 1:books
        from = Symbol("Agent_", rand(1:agts))
        to = Symbol("Agent_", rand(1:agts))
        while to == from
            to = Symbol("Agent_", rand(1:agts))
        end
