...
# Each MicroLedger is a matrix of Float64 with columns as accounts
struct MicroLedger
...
# Evaluate macro bookings into two micro ledgers and macro matrix
function evaluate_macro_bookings(bookings::Vector{MacroBooking}, num_accounts::Int)
    ...
    macro_matrix = zeros(Float64, num_rows, 1)  # only amount column for now
...
# Check colimit / glueing condition per micro ledger
...
# Construct the pattern diagram for one macro booking
