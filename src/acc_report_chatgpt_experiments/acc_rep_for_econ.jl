...
Pattern (kategorisch):")
    ...
    idx2 = split(String(b.credit.id), "_")[end]
    println(
        "  Investor           : Zahlung von ", b.amount, " zwischen Agenten Agent_" * idx1, "\n",
        " (Sender) und Agent_" * idx2, " (Empfänger), über Konten ", b.debit.id, " und ", b.credit.id, "und Agent_", idx2, "\n",
        " (Empfänger), über Konten ", b.debit.id, " und ", b.credit.id, "\n",
        " (Sender) und ", String(b.credit.id), " (Empfänger), über Konten ", b.debit.id, " und ", b.credit.id)
...
function example_macro(numa, numb)
    ...
    for i in 1:numb
        from = Symbol("Agent_", rand(1:numa))
        to = Symbol("Agent_", rand(1:numa))
        while to == from
            to = Symbol("Agent_", rand(1:numa))
        end
