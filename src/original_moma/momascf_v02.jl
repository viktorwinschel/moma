# ---------------------------------------------------------------------
# MoMa
# Monetery Makrobuchhaltung in Supply Chain Finance (MoMaSCF)
...
using CSV
# ---------------------------------------------------------------------
#= 5 agents,14:(3,3,6,3,5):accounts,bookings
...
# Parameter
Pars = (
    InvestmentLen=10, # Int Länge und Anzahl (später) von Investitionen
    # Vektor implementiert Speicher (als FILO Stack) für die
    # Rückzahlung von Investitionen und Bezahlung von Löhnen
    LabResourceRatio=0.2, # Float64 Verhältnis Löhne zu Ressourcen
    ConsumRatioRes=0.8, # Float64 Verhältnis Ressourcen zu Konsum
    ConsumRatioLab=0.95, # Float64 Verhältnis Löhne zu Konsum
    ConsumRatioCap=0.6, # Float64 Verhältnis Dividenden zu Konsum
    MarkUp=0.5, # Float64 Markup
    DivRateDiff=0.15, # Float64 Dividendenrate für Unternehmen
    DivRateBank=0.4, # Float64 Dividendenrate für Banken    
    WindFallProfit=0.5, # Float64 Windfall Profit
    sigA=20.0,  # Float64 sigA lern Parameter Investitionen (demand gap)
    sigB=480.0, # Float64 sigB lern Parameter Investitionen (demand gap)
    sigC=200.0, # Float64 sigC lern Parameter Investitionen (demand gap)
    ResourcePrice=25.0, # Float64 Preis Ressourcen
    LaborPrice=12.0, # Float64 Preis Löhne
    InitialGoodPrice=30.0, # Float64 Anfangspreis Produkte
    LabResSubstProd=0.75,  # Float64 Elastizität der Substitution
    ScaleProd=0.42, # Float64 Skalierungsparameter 
    DecayGoodLab=0.95, # Float64 Abschreibung Labor
    DecayGoodRes=0.7, # Float64 Abschreibung Ressourcen
    DecayGoodCap=0.6, # Float64 Abschreibung Kapital
    ReNewRes=100.0, # Float64 neue Ressourcen jede Periode
    ReNewLab=100.0, # Float64 neue Labor jede Periode
);
# ---------------------------------------------------------------------
# Zustandsvariablen 
# ---------------------------------------------------------------------
...
abstract type Nominal end
abstract type Real end
=#
...
# ---------------------------------------------------------------------
    # - Lab
    # --- Bank account (asset)
    # M3_Lab(D,_)ex-Lab-Com-Bank: money inflow selling labour
    ...
    # M3_Lab(_,C)ex-Lab-Com-Bank: labour outflow selling labour
    outAccLabLab = WagesPayment / pars.LaborPrice                           
    # state update
    stateNew.AccLabLab = 
        state.AccLabLab + inAccLabLab - outAccLabLab      
    # --- Good account (asset)
    # M4_Lab(D,_)ex-Lab-Com-Bank: good inflow buying good
...
# ---------------------------------------------------------------------
    # - Res
    # --- Bank account (asset)
    # M1_Res(D,_)ex-Res-Com-Bank: money inflow selling resources
    inAccResBank = InvestmentRes                                            
    # M2_Res(_,C)ex-Res-Com-Bank: money outflow buying good
    ...
    # M1_Res(_,C)ex-Res-Com-Bank: resources outflow selling resources
    outAccResRes = InvestmentRes / pars.ResourcePrice                      
    # state update
    stateNew.AccResRes = 
        state.AccResRes + inAccResRes - outAccResRes      
    # --- Good account (asset)
    # M2_Res(D,_)ex-Res-Com-Bank: good inflow buying good
...
# ---------------------------------------------------------------------
    ...
    # M6_Com(D,_)ex-Cap-Com-Bank: dividend outflow dividend payment
    outAccComDiv = DividendPayment                                         
    # state update
    stateNew.AccComDiv = 
        state.AccComDiv + inAccComDiv - outAccComDiv      
    # --- Loan account (liability)
    # M5_Com(_,C)ex-Com-Bank: loan increase investment
    inAccComLoan = Investment                                               
    # M7_Com(D,_)ex-Com-Bank: loan decrease repayment
    outAccComLoan = RepaysPayment                                           
    # state update
    stateNew.AccComLoan = 
        state.AccComLoan + inAccComLoan - outAccComLoan   
    # --- Resource account (asset)
    # M1_Com(D,_)ex-Res-Com-Bank: resource inflow buying resources
    ...
    # --- Labour account (asset)
    # M3_Com(D,_)ex-Lab-Com-Bank: labour inflow buying labour
